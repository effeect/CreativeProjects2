...
//Set the value of the enemy to 1 to instantly 
trump::trump(float x, float y) : enemy(5, 100, 0, 0, 0) { //The values of the enemy
	xPos = x;
	yPos = y;
}
...
void trump::draw() {
	...
	timeMeasure += ofGetFrameRate() * 3; //Default;
	//timeMeasure += 1000; //Debug Option to skip through animations much quicker

	//Health Bar Calculation
	mappedEnemy = ofMap(health, 0, 5, 0, ofGetWidth() - 70);
